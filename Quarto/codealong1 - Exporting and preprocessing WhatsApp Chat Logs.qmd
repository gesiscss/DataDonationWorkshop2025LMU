...
In the most basic form, a chat log file consists of a list of messages with **3 part each**
...
3. <span class="bg-yellow">**Message Body:**</span> The message that was send. Includes emoji, smileys, links, locations. For voice or video calls, an indicator is present in the text file. The same is true for sent media files or attached files.
...
WhatsApp adds some information into chat logs as system messages. These are problematic for parsing because they do have a timestamp but not sender. 
...
For both phones, the timestamp depends mainly on the setting of the exporting phone. The most frequent formats chosen are the 24h format and the AM/PM format. The 24h format is more common in Europe, while the AM/PM format is more common in the US. However, even for the same timestamps, iOS and Android can have different formattings. For example, the following timestamps are the same but formatted differently:
...
To effectively parse a WhatsApp chat log, we need to know the operating system of the device that exported the chat log, the language setting of the exporting phone, the date-time format of the operating phone and whether the file was exported including media files or not. Because WhatsApp data donations do not provide us with this meta-information, we need to infer it from the structure of the chat logs automatically, and then use the settings to parse the data.
...
To do this by hand, we would need to:

- build our own list of Regexes to detect WhatsApp System Messages
- RegExes detect all varieties of timestamps
- RegExes to detect media files
- Do all of this for multiple operating systems, languages, time settings and for exports including and not including media files
...
:::{callout-tip}
If you're using the GitHub dev version of `WhatsR`, the parsed dataframe also contains the inferred phone language, detected operating system, and time the exported chat log was parsed. The CRAN version of the package does not contain this feature yet.
:::
...
You can now use the `parse_chat` function to parse the example chat files or your own chat files.
